...
def test_player_keeps_getting_another_hand_until_can_move(people, turtles):
    cards = [Card(i, 'RED', 'MINUS') for i in range(HAND_SIZE)]
    cards += [Card(i, 'RAINBOW', 'MINUS')
              for i in range(HAND_SIZE, 2*HAND_SIZE)]
    cards += [Card(i, 'GREEN', 'PLUS')
              for i in range(2*HAND_SIZE, 4*HAND_SIZE)]
...
def test_raises_when_plays_arrow_incorrect_color(people, cards, turtles):
    player = people[0]
...
def test_raises_when_plays_arrow_arrow_incorrect_color(people, cards, turtles):
    ...
    with pytest.raises(ValueError):
        game.play(player, Action(Card(4*HAND_SIZE, 'RAINBOW', 'ARROW_ARROW'),
                  'RED'))
